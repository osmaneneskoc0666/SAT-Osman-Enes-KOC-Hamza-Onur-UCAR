...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $ozet   = $_POST['ozet'];
    $icerik = $_POST['icerik'];
    // Session'da ID hangi isimle tutuluyorsa (genelde 'id' veya 'user_id')
    ...
    if (isset($_FILES['dosya']) && $_FILES['dosya']['error'] == 0) {
        ...
        if (in_array($uzanti, $izinli)) {
            ...
            if (!is_dir('images')) mkdir('images'); // Klasör yoksa oluştur
    ...
    // 2. DB'ye Ekleme
    $sql = "INSERT INTO blog (baslik, ozet, icerik, resim_url, yazar_id, tarih, onay_durumu) VALUES (?, ?, ?, ?, ?, NOW(), 'onay_bekliyor')";
    $stmt = $db->prepare($sql);
    
    if ($stmt->execute([$baslik, $ozet, $icerik, $resim_url, $yazar_id])) {
        $mesaj = '<div class="alert success">✅ Yazınız başarıyla gönderildi! Editör onayından sonra yayınlanacaktır.</div>';
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Blog Yazısı Gönder</title>
...
<div class="container">
    <?php echo $mesaj; ?>
    <h2><i class="fas fa-edit" style="color:#228B22;"></i> Blog Yazısı Gönder</h2>
    <form method="post" enctype="multipart/form-data">
        ...
        <button type="submit"><i class="fas fa-paper-plane"></i> Yazıyı Gönder</button>
    </form>
</div>
